...
export function MovementHeatMapCard({
  ...
  const flowData = useMemo(() => {
    ...
    movements.forEach((mov) => {
      ...
      // Collect event type information from From_Sub_Location and To_Sub_Location
      if (mov.From_Sub_Location) {
        flow.fromEvents.add(mov.From_Sub_Location);
      }
      if (mov.To_Sub_Location) {
        flow.toEvents.add(mov.To_Sub_Location);
      }
    });
